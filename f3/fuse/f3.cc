...
static int f3_mark_write_done(int fd) {
    ...
    res = fsetxattr(newfd, "user.f3.writedone", done, sizeof("done"), 0);
    if (res == -1) {
        saverr = errno;
    }
...
static void sfs_read(fuse_req_t req, fuse_ino_t ino, size_t size, off_t off,
    ...
    while ((unsigned long)stat.st_size < (off + size) && (off + size) < inode.target_size && timeout > 0) {
        F3_LOG("%s: waiting... %ld %lu %lu\n", __func__, stat.st_size, inode.target_size, (unsigned long)time(NULL));
        //sleep(1);
        usleep(500000);
...
static void do_write_buf(fuse_req_t req, size_t size, off_t off,
    ...
    if (res < 0)
        ...
        if (inode.is_id) {
            ...
            if ((off + res) > (long unsigned int)inode.size) {
                inode.size = off + res;
            }
